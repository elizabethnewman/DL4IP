...
options.data.size = [512,512];

% choose dictionary
options.dictionary.filename = '2023_10_24--earth_dictionary.mat';

% operator
options.AOperator.name   = 'superresolution';
options.AOperator.params = {'downSamplingRatio', 8, 'numberOfLowRes', 10};
...
% noiseLevel = 1e-8;
% b = b + norm(b) * noiseLevel * randn(size(b));

%% nonnegative mapping
% [a,c]   = deal(0.2975,-0.05);
[a,c]   = deal(0.325,0);
...
numberOfLowRes  = length(results.probInfo.btrue);
lowResImageSize = results.probInfo.lowResImageSize;
for j = 1:numberOfLowRes
    fig = figure(1); clf;
    imagesc(reshape(results.probInfo.btrue{j},lowResImageSize)); colormap parula;
    axis('off')
    axis('image')
    exportgraphics(fig,[saveDirName,'lowres_',num2str(j),'.png'],'BackgroundColor','none')
end
