...
z = rand(size(AD,2),1);
options.MRNSDSparsity.lambda          = 1e-4;
options.MRNSDSparsity.options.MaxIter = 100;
options.MRNSDSparsity.options.x0      = w(z);
...
numberOfLowRes  = length(probInfo.btrue);
lowResImageSize = probInfo.lowResImageSize;
for j = 1:numberOfLowRes
    fig = figure(1); clf;
    imagesc(reshape(probInfo.btrue{j},lowResImageSize)); colormap parula;
    axis('off')
    axis('image')
