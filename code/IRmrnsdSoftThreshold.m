function [X, info] = IRmrnsdSoftThreshold(A, b, lambda, varargin)
...
%      nnz      - number of nonzeros in solution per iteration
...
RnrmNum     = zeros(MaxIter,1);
alphas      = zeros(MaxIter,1);
NE_Rnrm     = zeros(MaxIter,1);
nnzCount    = zeros(MaxIter,1);
...
% ------------------------------------------ %
% Initial Evaluation
info.initialEval.Rnrm    = norm(r) / norm(b(:));
info.initialEval.RnrmNum = norm(r);
info.initialEval.nnzX    = nnz(x);
% ------------------------------------------ %
...
for k = 1:MaxIter
    ...
    % if ~exist('superres_mrnsd','dir'), mkdir('superres_mrnsd'); end
    % 
    % tmp3 = reshape(x,400,[]);
    ...
    % axis('off'); axis('image');
    % set(gca,'FontSize',24)
    % exportgraphics(fig,['superres_mrnsd/img_',num2str(k-1),'.png'])
    ...
    % find best choice of \alpha for MRNSD
    % gamma   = -g(:)' * s(:);
    % theta   = gamma / (u(:)' * u(:));
    % neg_ind = s < 0;
    % bnd     = min(-x(neg_ind) ./ s(neg_ind));
    % if ~isempty(bnd)
    %     alpha = min(theta, bnd);
    % else
    %     alpha = theta;
    ...
    h = @(alpha) 0.5 * norm(A * softThreshold(x + alpha*s, alpha * lambda) - b,'fro')^2;

    % [k,min(-x(neg_ind) ./ s(neg_ind))]
    neg_ind = s < -lambda;
    bb = min(-x(neg_ind) ./ (s(neg_ind) + lambda));
    if isempty(bb)
        bb = 1e4;
    end
    alpha = fminbnd(h,0,bb);
    % fminbnd
    if isempty(alpha)
        alpha = min( -x(neg_ind) ./ s(neg_ind) );
    end


  
    x = softThreshold(x + alpha*s, alpha * lambda); % E. Newman change: added soft threshold

    if any(x < 0), disp('negative values detected'); end
    ...
    if errornorms
        ...
        if Enrm(k)<BestEnrm
            ...
            BestReg.Xnrm = Xnrm(k);
            BestReg.Rnrm = Rnrm(k);
            BestReg.alphas = alphas(k);
            BestReg.RnrmNum = RnrmNum(k);
...
if nargout==2
  ...
  info.Rnrm             = Rnrm(1:k);
  info.RnrmNum          = RnrmNum(1:k);
  info.alphas           = alphas(1:k);
  info.NE_Rnrm          = NE_Rnrm(1:k);
  info.Xnrm             = Xnrm(1:k);
  info.nnzX             = nnzCount(1:k);
  if errornorms
    info.Enrm    = Enrm(1:k);
...
function[x] = softThreshold(x, alpha)
